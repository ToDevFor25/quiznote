...
_Last revised: May 2026 (CSS extraction now 4 of ~6 clusters in `qn-theme.css` — summary, start screen, play-screen chassis, modal — inline CSS per module down ~41% (765→450), all deployed + QA'd live; buttons/cards clusters remain. Feedback toast centralized with Option-2 placement (praise over staff, retry/reveal lower), wired to scale-degrees only so far — other 8 pending rollout. Key-sig clef-clearance made proportional in `qn-staff.js` (`lineGap * 4.5`), fixing a flats-overlap-clef bug in scale-degrees (key-signatures unchanged, time-signatures key sig +18px, pin-able). Earlier this session: 3 CSS clusters; all 9 quit dialogs unified to Option 1; key-signatures + scales markup fix. Prior: Shared-CSS architecture — `qn-theme.css` added (tokens + prompt + tiles, all 9 migrated); "quiet A" prompt; `QN.ui.confirm` shared modal component (`qn-profile.js` v1.7.0); §8 rendered-result audit rule; `BUILD_LOG.md` cleaned + archived. Earlier still: roster expanded to 24; path-first three-ring IA; device reset.) Supersedes the v1 doc that locked the seven-module roster._
...
- **`qn-ui.js` (folded into `qn-profile.js`'s `QN.ui`, not a separate file).** Shared widgets live under `QN.ui`: `chip` (profile chip) and `confirm` (the shared quit/confirm modal component, added v1.7.0 — see §6 modal note). The originally-planned standalone `qn-ui.js` was unnecessary; `QN.ui` is the home.
...
- **A non-additive change (rename/retype/restructure) requires a migration — and the hook is not built yet.** There is currently NO `schemaVersion` stamp on the stored blobs (`QN.version` versions the CODE, not the data). Before the first breaking change, add a `schemaVersion` field and a migrate-on-read step (if stored version < expected, transform old→new before use; migrations idempotent). Deferred deliberately while data is simple and additive; documented here so it isn't forgotten. Retrofitting versioning AFTER unversioned data is in the wild is the painful path.
...
12. **Modes** — Dorian, Phrygian, Lydian, Mixolydian, Aeolian, Locrian. *Stretch.*
...
19. **Ear: Intervals** — hear two notes, identify the interval. *Planned.*
...
7. **Ear: Intervals, Ear: Chord Quality** (§5 #19–20) — first ear-training pair; needs the multi-timbre audio (Tier 3 audio plan).
8. **Stretch tier** (§5 #12,13,21,22,23) — Modes, Transposition, Melodic/Rhythmic Dictation, Score Reading — demand-contingent.
...
- **Consolidation pass (in progress):** ✓ **Clef rendering consolidated** — all six staff modules call `NH.staff.buildClef()`. ✓ **Play-staff accidental glyphs consolidated** — `NH.staff.buildNoteAccidental()` (qn-staff.js v1.2.0), used by the five staff modules. ✓ **`qn-music.js` created (v1.0.0, forward-looking)** — superset pitch helpers; NEW modules use `NH.music`; existing modules keep inline copies until edited (option A). ✓ **`qn-theme.css` created (v1.0.0, May 2026) — first shared CSS file.** Design tokens + canonical question prompt + answer-tile system extracted; all 9 modules migrated (link added, duplicated rules stripped). The question prompt was standardized to the "quiet A" look (sentence-case, centered, 22px, fixed-height zone so the staff never jumps) and prompt wording normalized to question form across modules. ✓ **CSS extraction — 4 of ~6 clusters done (May 2026):** summary, start screen, play-screen chassis, and modal extracted to `qn-theme.css`; inline CSS per module down ~41% (note-names 765→450), all deployed and QA'd live. ✓ **Feedback toast centralized (Option-2 placement)** — shared toast now in `qn-theme.css`, wired to scale-degrees only so far (other 8 keep inline, pending rollout); scales' different feedback model stays separate. ✓ **Modal arrangement unified** — all 9 quit dialogs show Option 1 (Quit ghost/left, Keep green/right); key-signatures + scales fixed by markup edit. ✓ **Key-sig clef-clearance made proportional** — `qn-staff.js` `buildAccidentals`/`buildStaff` startX changed from fixed 72/70 to `lineGap * 4.5/4.4` (clef width scales with lineGap); fixed a flats-overlapping-clef bug in scale-degrees, key-signatures unchanged, time-signatures key sig shifts +18px (pin-able via new `accStartX` override). **Still pending (sequenced):** (1) roll the shared toast to the other 8 modules (QA per module — card heights differ); (2) extract remaining CSS clusters — buttons (~81), cards (~72), ~150 lines; (3) roll `QN.ui.confirm` to the other 7 modules + retire note-names' `showConfirm` (modal component still 2-of-9); (4) time-signatures prompt-layout conversion + scales tile reconciliation; (5) add notehead rendering to `qn-staff.js`; (6) sampled-piano audio (Tier 2). The rest of each renderer stays module-specific.
- **`schemaVersion` migration hook — pull forward (flagged May 2026).** Confirmed by audit: there is still NO `schemaVersion` stamp on stored data (`QN.version` versions code, not data). With 15 more modules and cloud sync coming, the odds of a non-additive data change rise. Recommendation: add the `schemaVersion` field + migrate-on-read step NOW while data is all-additive, rather than retrofitting after unversioned data is in the wild (the painful path, per § 4 storage playbook). This is the highest-leverage deferred infra item and should precede backend work.
